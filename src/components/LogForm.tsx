    ...
    <div className="bg-zinc-900 border border-zinc-800 rounded-3xl p-6 sm:p-8 shadow-xl">
      <div className="flex items-center justify-between mb-8">
        <h2 className="text-xl font-medium text-zinc-100 flex items-center gap-2">
          <Syringe className="w-5 h-5 text-emerald-400" /> Registrar Dose
        </h2>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        <div className="space-y-4">
          <div className="space-y-2">
            <label className="text-sm font-medium text-zinc-400 flex items-center gap-2">
              <Clock className="w-4 h-4" /> Horário da Aplicação
            </label>
            <input 
              type="time" 
              value={time} 
              onChange={e => setTime(e.target.value)}
              className="w-full bg-zinc-950 border border-zinc-800 rounded-xl px-4 py-3 text-zinc-100 focus:outline-none focus:ring-2 focus:ring-emerald-500/50 font-mono"
              required
            />
          </div>
        ...
        <div className="space-y-3">
          <label className="text-sm font-medium text-zinc-400 flex justify-between items-center">
            <span className="flex items-center gap-2"><MapPin className="w-4 h-4" /> Localidade</span>
            {site !== recommendedSite && (
               <span className="text-[10px] text-emerald-400/80 bg-emerald-500/10 px-2 py-0.5 rounded-full uppercase tracking-wider font-semibold cursor-pointer transition hover:bg-emerald-500/20" onClick={() => setSite(recommendedSite as InjectionSite)}>Sugestão: {recommendedSite}</span>
            )}
          </label>
        ...
        <div className="space-y-3">
          <label className="text-sm font-medium text-zinc-400">Protocolo</label>
          <div className="grid grid-cols-2 gap-2">
            {PROTOCOLS.map(p => (
              <button
                key={p}
        ...
        <div className="space-y-2">
          <label className="text-sm font-medium text-zinc-400 flex items-center gap-2">
            <NotebookPen className="w-4 h-4" /> Notas Diárias (Opcional)
          </label>
          <input 
            type="text" 
            value={notes}
            ...
            className="w-full bg-zinc-950 border border-zinc-800 rounded-xl px-4 py-3 text-zinc-100 placeholder:text-zinc-600 text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500/50"
          />
        </div>

        <button 
          type="submit"
          className="w-full bg-emerald-500 hover:bg-emerald-400 text-zinc-950 font-semibold py-4 rounded-xl transition-all duration-200 flex items-center justify-center gap-2 shadow-lg shadow-emerald-500/20 hover:shadow-emerald-500/30 active:scale-[0.98]"
        >
